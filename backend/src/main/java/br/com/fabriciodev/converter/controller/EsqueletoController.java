...
import br.com.fabriciodev.converter.service.EsqueletoService;
import lombok.RequiredArgsConstructor;

@RestController
@RequestMapping("/api/esqueletos")
@RequiredArgsConstructor
public class EsqueletoController extends BaseApiController {

    private final EsqueletoService service;

    @GetMapping()
    public ResponseEntity<?> findAll() {
        try {
            List<EsqueletoDTO> data = service.findAll();
    ...
    public ResponseEntity<?> search(@RequestBody FiltroDTO filtro) {
        try {
            ...
            List<EsqueletoDTO> data = service.search(filtro, dataFiltro);
    ...
    public ResponseEntity<?> findActive() {
        try {
            List<EsqueletoDTO> data = service.findActive();
            return ResponseHandler.renderJSON(
                    new RetornoListaJsonDTO(
                            BuildResult.json(data),
                            data.size()));
    ...
    public ResponseEntity<?> get(@PathVariable Integer id) {
        try {
            EsqueletoDTO dto = service.get(id);
    ...
    @PostMapping("/create")
    public ResponseEntity<?> create(@RequestBody EsqueletoDTO dto) {
        try {
            EsqueletoDTO esqueleto = service.create(dto);
            return ResponseHandler.renderJSON(
                    new RetornoSucessoJsonDTO(GenericMessages.get("generic.sucesso"), esqueleto));
        } catch (Exception ex) {
            return error(ex);
        }
    }
    ...
    public ResponseEntity<?> update(@PathVariable Integer id, @RequestBody EsqueletoDTO dto) {
        try {
            EsqueletoDTO esqueleto = service.update(id, dto);
            return ResponseHandler.renderJSON(
                    new RetornoSucessoJsonDTO(GenericMessages.get("generic.sucesso"), esqueleto));
        } catch (Exception ex) {
            return error(ex);
